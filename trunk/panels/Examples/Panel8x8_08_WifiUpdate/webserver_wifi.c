 ...
 *****************************************************************************/
#include "../WiShield/uip.h"
#include <string.h>
#include "webserver_wifi.h"
#include "../WiShield/config.h"

static int handle_connection(struct webserver_state *s);

extern void ClearPanelBuffer(void);
extern void WritePanelByte(byte);
extern void NewPanelMessage(void);

void webserver_init(void)
{
	uip_listen(HTONS(80));
        // CRJ 2009-10-07 Added to allow toggling D8
        pinMode(8,1);
}
                ...
                if (s->inputbuf[1]=='0') {
                  ...
                  while (!exitflag) {
                   ...
                   for (;j<i;j++) {
                     b = s->inputbuf[j];
                     if (b=='&') {i=0; exitflag=1;}
                     else WritePanelByte(b);
                   }
                   if (!exitflag) {PSOCK_READBUF(&s->p);}
                  }
                  NewPanelMessage();
                }
		else PSOCK_CLOSE_EXIT(&s->p);
	}
